...
export default async function PostPage({ params }: PageProps) {
  ...
  const badgeColor = categoryColors[post.category] || "bg-deep-sage";
    ...
    <div className="bg-cream">
      ...
      <article className="max-w-2xl mx-auto px-4 py-8">
            ...
            {post.category === "cooking" && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-charcoal bg-cream border-2 border-sage rounded-full hover:bg-deep-sage hover:text-white transition-all"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Recipe Card - for cooking posts with recipe data (at end of content).
            No "Made this?" CTA here: it duplicated the CommentsPreview card
            right below it, and the stacked look-alike CTAs buried everything. */}
        ...
        {JOURNEY_NEXT[slug] && (
          <div className="bg-white rounded-2xl shadow-md p-6 mb-8 border-2 border-terracotta/40 text-center">
        ...
        {/* Mid-Post Email Signup CTA. Suppressed on journey posts: the journey
            banner already carries the next action there, and a second email ask
            in the same stack competes with it (BottomEmailCTA still renders at
            the very bottom of the page). */}
        {!JOURNEY_NEXT[slug] && <PostEmailSignup category={post.category} />}
        ...
        <footer className="mt-6 pt-6 border-t border-light-sage">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <Link
              href={`/${post.category}`}
              className="px-4 py-2 border-2 border-sage text-deep-sage font-semibold rounded-full hover:bg-deep-sage hover:text-white transition-all"
            >
              More {categoryLabel}
            </Link>
          </div>
