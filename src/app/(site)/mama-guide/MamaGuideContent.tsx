  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email, firstName, source, segment: "mama-life" }),
      });
    ...
    return (
      <div className="bg-light-sage/30 rounded-lg p-6 text-center">
        ...
        <div className="mt-4 pt-4 border-t border-sage/20">
          ...
          >
            Get the Free Sourdough Starter Guide
          </Link>
        </div>
      </div>
    );
...
interface MamaGuideContentProps {
  totalPosts: number;
}

export function MamaGuideContent({ totalPosts }: MamaGuideContentProps) {
  return (
    <>
      {/* Hero Section */}
